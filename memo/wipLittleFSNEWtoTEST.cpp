...
// TX state machine for non-blocking loop
enum TXState {
    WAITING_FOR_TX,
    WARMING_UP,
    TRANSMITTING,
...
TXState txState = WAITING_FOR_TX;
...
void loop()
{
    // If interrupted, skip any current TX
    if (interruptWSPRcurrentTX)
    {
        warmingup = false;
        txState = WAITING_FOR_TX;
        delay(5);
        yield();
        return;
    }
    ...
    if (isFirstIteration)
    {
        ...
        // Print initial diagnostic info once
    ...
    // Once per second: update countdown
    ...
    switch (txState)
    {
        ...
        case TRANSMITTING:
        {
            ...
            // Print next TX time after TX completes
            Serial.print("🕑 Next TX Time: ");
            Serial.println(convertPosixToHHMMSS(nextPosixTxTime));
            displaySelecetdBandInformation(selectedBandIndex);
            break;
...
void transmitWSPR_core0(void *parameter)
{
    ...
    for (int i = 0; i < SYMBOL_COUNT; i++)
    {
        if (interruptWSPRcurrentTX)
        {
            Serial.println("⚠️ TX interrupted — exiting WSPR transmission.");
            break;
        }
        ...
        // Precise 683 ms per symbol using FreeRTOS delay
    ...
    vTaskDelete(NULL); // cleanly delete this task from Core 0
}
